...
df = pd.read_csv('india_dams_details_2.csv',sep=',')

name_list = df['Name'].tolist()
place_list = df['Place'].tolist()
river_list = df['River'].tolist()
...
for item in name_list:
    ...
    river = str(river_list[name_list.index(item)])

    place_html = '' if (place=='nan') else "<p style=\"font-size:12px\">Location: "+place+"</p>"
    river_html = '' if (river=='nan') else "<p style=\"font-size:12px\">River: "+river+"</p>"

    html="<div style=\"@import url('https://fonts.googleapis.com/css?family=Roboto+Condensed');font-family: 'Roboto Condensed', sans-serif;\">"+"<h5>"+item+"</h5>"+place_html+river_html

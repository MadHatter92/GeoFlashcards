...
df = pd.read_csv('british_india_treaties_details.csv')

name_list = df['Name of the Treaty'].tolist()
year_list = df['Year'].tolist()
description_list = df['Importance'].tolist()
...
for item in name_list:
    ...
    description_html = '' if (year=='nan') else "<p style=\"font-size:12px\">"+description+"</p>"


    html="<div style=\"@import url('https://fonts.googleapis.com/css?family=Roboto+Condensed');font-family: 'Roboto Condensed', sans-serif;\">"+"<h5>"+item+"</h5>"+year_html+description_html

    # resolution, width, height = 10, 50, 25
    iframe = folium.IFrame(html, width=200, height=200)
    popup = folium.Popup(iframe, max_width=1000)
    icon = folium.Icon(color="cadetblue", icon="pencil-square-o", prefix='fa')
    marker = folium.Marker(location=[lat, lon], tooltip=item, popup=popup, icon=icon)
    marker.add_to(m)

m.save(f'../map_british_india_treaties.html')
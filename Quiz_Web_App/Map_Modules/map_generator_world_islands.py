...
def generate_map():
    ...
    data_frame = pd.read_csv('World_Geo_Markers/world_islands.csv')
    ...
    for item in name_list:
        ...
        icon = folium.Icon(color='blue', icon_color='white', icon='compass', prefix='fa')
        marker = folium.Marker(location=[lat, lon], tooltip=tooltip, icon=icon)
        marker.add_to(m)
        print('Added '+ item)

        m.save(f'../map_world_islands.html')
